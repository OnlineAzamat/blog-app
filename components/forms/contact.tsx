...
function ContactForm() {
  ...
  function onSubmit(values: z.infer<typeof contactSchema>) {
    setIsLoading(true);
    const telegramBotId = process.env.NEXT_PUBLIC_TELEGRAM_BOT_API_TOKEN;
...
📝 Message: ${values.message}`
    ...
    }).then(() => form.reset()).catch(err => console.error(err)).finally(() => setIsLoading(false));
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormControl>
                <Textarea className="resize-none h-32" placeholder="Ask question or just say Hi" disabled={isLoading} { ...field } />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
